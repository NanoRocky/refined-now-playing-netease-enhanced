...
export function ProgressbarPreview(props: {
  ...
  const [currentNonInterludeIndex, setCurrentNonInterludeIndex] =
    useState<any>(0);
  ...
  const onLyricsUpdate = (e: any) => {
    ...
    setNonInterludeCount(
      e.detail.lyrics.filter((l: any) => l.originalLyric).length,
    );
  };
  useEffect(() => {
    if (window.currentLyrics) {
      ...
      setNonInterludeCount(
        currentLyrics.filter((l: any) => l.originalLyric).length,
      );
    }
