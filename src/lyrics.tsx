...
export function Lyrics(props: {
  ...
  const isPureMusic =
    ...
    (lyrics.length === 1 ||
      (lyrics.length <= 10 &&
        lyrics.some((x: any) => (x.originalLyric ?? "").includes("纯音乐"))));
  ...
  useEffect(() => {
    ...
    for (let i = visualCurrent - 1; i >= 0; i--) {
      ...
      setRotateTransform(
        transforms[i],
        transforms[visualCurrent].top - transforms[i].top,
        heightOfItems.current[i] * transforms[i].scale,
      );
    }
    ...
    for (let i = visualCurrent + 1; i < lyrics.length; i++) {
      ...
      if (lyrics[i].isBG) {
        ...
        transforms[i].top =
          prevLineTop + prevLineHeight + bgSpace + bgLineEstimatedHeight / 2;
        ...
        if (lyrics[i - 1].isBG) {
          ...
          if (prevNonBGIndex >= 0) {
            const prevNonBGHeight =
              heightOfItems.current[prevNonBGIndex] *
              transforms[prevNonBGIndex].scale;
            transforms[i].top =
              transforms[prevNonBGIndex].top + prevNonBGHeight + space;
          } else {
          ...
          // 上一行是正常行
          transforms[i].top =
            transforms[i - 1].top + previousScaledHeight + space;
        }
      }
    ...
    transforms[lyrics.length].delay = delayByOffset(
      lyrics.length - visualCurrent,
    );
    setRotateTransform(
      transforms[lyrics.length],
      transforms[visualCurrent].top - transforms[lyrics.length].top,
